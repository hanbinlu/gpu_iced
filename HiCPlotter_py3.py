...
def read_HiCdata(filename,header=0,footer=0,clean_nans=True,smooth_noise=0.5,ins_window=5,rel_window=8,plotInsulation=True,plotTadDomains=False,randomBins=False):
	...
	if not randomBins and len(matrix[:,1]) != len(matrix[1,:]):
		...
		print('unbalanced matrix('+filename+')! input should be a square matrix', file=sys.stderr)
		raise SystemExit
...
def read_sparseHiCdata(filename,chromosome,bedFile,startBin,endBin,wholeGenome=False,smooth_noise=0.5,ins_window=5,rel_window=8,plotInsulation=True,plotTadDomains=False,randomBins=False):
	...
	try:
		...
		print('cannot open', bedFile, file=sys.stderr)
		raise SystemExit
	
	for line in bed.readlines():
		tags = line.strip().split("\t")
		if tags[0]=='chrM':continue
		if tags[0] not in list(chromosomes.keys()):
			chromosomes[tags[0]]=[]
	...
	if type(filename) == type(''):
		...
		mtx = filename[start:end+1,start:end+1]
...
def read_bedGraph(filename,resolution,chromosome): # add stopping after certain chromosome passed
	...
	if len(y_scores) !=0 and len(y_scores)!=len(x_scores):
		print('BedGraph('+filename+') has some missing values', file=sys.stderr)
		raise SystemExit
...
def read_peakFile(filename,resolution,chromosome): # add stopping after certain chromosome passed
	...
	if len(origin_y) !=0 and len(origin_x)!=len(origin_y):
		print('Peak file ('+filename+') has some missing values', file=sys.stderr)
		raise SystemExit
	...
	for line in fone:
		...
		if tags[0]==chromosome and int(tags[1])>start and int(tags[2]) < end:
			...
			for x in range(1,len(cols)):
				if x % 2 == 1 and cols[x].replace('[','').replace(']','').replace(' ','') not in list(y_dict.keys()):
					y_dict[cols[x].replace('[','').replace(']','').replace(' ','')]=[]
					y_dict[cols[x].replace('[','').replace(']','').replace(' ','')].append(float(cols[x-1].replace('[','').replace(']','').replace(' ','')))
				elif x % 2 == 1 :
					y_dict[cols[x].replace('[','').replace(']','').replace(' ','')].append(float(cols[x-1].replace('[','').replace(']','').replace(' ','')))
					
	if len(list(y_dict.keys())) ==0:
		print('Epilogos File('+filename+') has some missing values', file=sys.stderr)
		raise SystemExit
...
def read_genes(filename,resolution,chromosome,start,end):
	...
	for line in fone:
		tags = line.strip().split("\t")
		if tags[0]==chromosome:
			if int(tags[1]) >= start and int(tags[2]) <= end and tags[1]+'-'+tags[2] not in list(genes.keys()):
				...
				if len(row_list)==0:
					...
					if prev_start > int(tags[1]):
						...
						print('Gene File ('+filename+') is not sorted.', file=sys.stderr)
						raise SystemExit
					else:
						current_end = int(tags[2])
						current_start = int(tags[1])
						...
						for item in range(0,len(row_list)):
							if current_start > row_list[item]+minDist: 
								...
								if item+1 not in list(row_genes.keys()): row_genes[item+1]=[]
								row_genes[item+1].append(current_end)
								break
						if execute == 0:
							genes[tags[1]+'-'+tags[2]].append(len(row_list)+1)
							row_list.append(current_end)
							if len(row_list) not in list(row_genes.keys()): row_genes[len(row_list)]=[]
	...
	if len(list(genes.keys())) ==0:
		print('Gene File ('+filename+') has some missing values', file=sys.stderr)
		raise SystemExit
...
def insulation(matrix,w=5,tadRange=10,triple=False,mstart=0):
	...
	if triple: pBorders=[x+mstart for x in pBorders]
...
def HiCplotter(files=[],names=[],resolution=100000,chromosome='',output='',histograms=[],histLabels=[],fillHist=[],histMax=[],verbose=False,fileHeader=0,fileFooter=0,matrixMax=0,histColors=[],barPlots=[],barLabels=[],plotGenes='',superImpose=False,\
			...
			smoothNoise=0.5,cleanNANs=True,plotTriangular=False,plotTadDomains=False,randomBins=False,wholeGenome=False,plotPublishedTadDomains=False,plotDomainsAsBars=False,imputed=False,barMax=[],spine=False,plotDomainTicks=True,triangularHeight=False,\
			highlights=0,highFile='',heatmapColor=3,highResolution=True,plotInsulation=False,plotCustomDomains=False,publishedTadDomainOrganism=True,customDomainsFile=[],compare=False,pair=False,domColors=[],oExtension='',geneLabels=True,dark=False, inline=True):
	...
	rlength = len(files)-1 if compare and not pair else 1
	...
	for exp in range(0,rlength):
		...
		if not tripleColumn:
			...
			if exp == 0 : mlength = len(matrix)
				...
				print('unbalanced matrix size of '+files[exp]+' compared to '+files[0]+' ! matrix sizes should be equal', file=sys.stderr)
				raise SystemExit
			
			matrix=matrix[start:end,start:end]
		else:
			if bedFile == '':
				print('an annotation bed file is required for triple-column sparse input.', file=sys.stderr)
				raise SystemExit
			matrix,nums,tricks,clength=read_sparseHiCdata(files,chromosome,bedFile,start,end,wholeGenome,smoothNoise,window,tadRange,plotInsulation,plotTadDomains,randomBins)
		...
		ax1 = plt.subplot2grid((numOfrows, 4*1), (0, exp*4), rowspan=4,colspan=4)

		ax1.set_title(('%s') % (name))
		if exp==0: 
			if not randomBins and not wholeGenome: ax1.set_ylabel('(interaction matrix) - %s Mb (resolution: %sKb)' % (chromosome , resolution/1000))
			elif randomBins: ax1.set_ylabel('(interaction matrix) - %s (Genomic Bins)' % (chromosome))
		...
		if not wholeGenome:
			...
			if heatmapColor < 5:
				with np.errstate(divide='ignore'): img=ax1.imshow((matrix),cmap=plt.get_cmap(cmaps[heatmapColor]),interpolation="nearest",extent=(int(start or 1) - 0.5,\
														  		  int(start or 1) + length - 0.5,int(start or 1) - 0.5,int(start or 1) + length - 0.5),aspect='auto')
			elif heatmapColor == 3:
				cmap = plt.get_cmap(cmaps[heatmapColor])
				cmap.set_over('black')
				...
				with np.errstate(divide='ignore'): img=ax1.imshow((matrix),interpolation="nearest",extent=(int(start or 1) - 0.5,\
														  		  int(start or 1) + length - 0.5,int(start or 1) - 0.5,int(start or 1) + length - 0.5),aspect='auto')
		...
		if numOfrows <= rowcounter and not randomBins and not wholeGenome: 
			...
			cbar = plt.colorbar(img, cax=cax, ticks=MultipleLocator(5.0), format="%.1f",orientation='horizontal',extendfrac='auto',spacing='uniform')
			plt.setp(ax1.get_xticklabels(), visible=False)
		...
		if wholeGenome and numOfrows > rowcounter:
			print('Whole genome can be plotted only as matrix - this feature will be improved in future releases', file=sys.stderr)
			raise SystemExit
		...
		if plotTriangular: 
			...
			ax2 = plt.subplot2grid((numOfrows, 4*1), (rowcounter, exp*4), rowspan=2,colspan=4,sharex=ax1)
		...
		if randomBins and numOfrows > rowcounter:
			print('Random bins data can be plotted only as matrix and triangular - this feature will be improved in future releases', file=sys.stderr)
			raise SystemExit
		...
		if len(plotGenes) > 0:
			...
			ax3 = plt.subplot2grid((numOfrows, 4*1), (rowcounter, exp*4), rowspan=2,colspan=4,sharex=ax1)
		...
		if len(histograms)>0:
			if not superImpose: # improve this part
				for x in range(0,len(histograms[0].split(','))):
					ax3 = plt.subplot2grid((numOfrows, 4*1), (rowcounter, exp*4), rowspan=1,colspan=4,sharex=ax1)
					ax3.get_yaxis().set_label_coords(-0.125,0.5)
					x_comps,x_comps2,y_comps,colors,texts = read_bedGraph(histograms.split(',')[x],resolution,chromosome)
					...
					if exp==0: 
						...
						if len(histMax)==0:
							...
							ax3.set_ylim(ymin,int(histMax.split(',')[x])+int(histMax.split(',')[x])/10)
						ax3.set_ylabel(histLabels.split(',')[x])
					else:
						if len(histMax)==0:
				...
				hfirst = histograms.split(',')[0]
				hsecond = histograms.split(',')[1]
				...
				if exp==0: 
					...
					if len(histMax)==0:
						...
						ax3.set_ylim(ymin,int(histMax.split(',')[0])+int(histMax.split(',')[0])/10)
					ax3.set_ylabel(histLabels.split(',')[0],color=firstcolor)
				else:
					if len(histMax)==0:
				...
				if exp==0: 
					...
					if len(histMax)==0:
						...
						ymaxlims.append(max(y_comps[ystart:yend]))
						ax3a.text(int(start or 1)+length-length/6,max(y_comps[ystart:yend])-max(y_comps[ystart:yend])/10,histLabels.split(',')[1],color=subcolor)
					else:
						ax3a.set_ylim(ymin,int(histMax.split(',')[1])+int(histMax.split(',')[1])/10)
						ax3a.text(int(start or 1)+length-length/6,int(histMax.split(',')[1])-int(histMax.split(',')[1])/10,histLabels.split(',')[1],color=subcolor)
					#ax3a.set_ylabel(histLabels.split(',')[1])
				else:
					if len(histMax)==0:
		...
		if plotInsulation: 
			...
			ax4 = plt.subplot2grid((numOfrows,4*1), (rowcounter, exp*4), rowspan=1,colspan=4,sharex=ax1)
			...
			if not tripleColumn:
				if dark: ax4.plot(list(range(0,len(nums))),nums,'white')
				else: ax4.plot(list(range(0,len(nums))),nums,'black')
				ax4.fill_between(list(range(0,len(nums))),nums,0,color='0.8')
			else:
		...
		if plotTadDomains: 
			...
			ax5 = plt.subplot2grid((numOfrows,4*1), (rowcounter, exp*4), rowspan=1,colspan=4,sharex=ax1)
	...
	if compare and not pair:
		...
		ax1 = plt.subplot2grid((numOfrows, 4*1), (0, (exp+1)*4), rowspan=4,colspan=4)
		with np.errstate(divide='ignore'): img=ax1.imshow((matrix),cmap=plt.get_cmap("bwr"),origin=orientation,interpolation="nearest",extent=(int(start or 1) - 0.5,\
														  		  int(start or 1) + length - 0.5,int(start or 1) - 0.5,int(start or 1) + length - 0.5),aspect='auto')
		
		ax1.set_ylim(int(start or 1) - 0.5,int(start or 1) + length - 0.5)
		ax1.set_xlim(int(start or 1) - 0.5,int(start or 1) + length - 0.5)
		ax1.set_title(('(%s / %s)') % (names[0],names[1]))
		...
		if plotTriangular: 
			...
			ax2 = plt.subplot2grid((numOfrows, 4*1), (crowcounter, (exp+1)*4), rowspan=2,colspan=4,sharex=ax1)
			dst=ndimage.rotate(matrix,45,order=0,reshape=True,prefilter=False,cval=0)
			matrix=[];
			if not triangularHeight: height=length/5
			else: 
		...
		if len(plotGenes) > 0:
			...
			ax3 = plt.subplot2grid((numOfrows, 4*1), (crowcounter, (exp+1)*4), rowspan=2,colspan=4,sharex=ax1)
			if exp==0: ax3.set_ylabel('Genes')
			ax3.get_yaxis().set_label_coords(-0.125,0.5)
			genes,trackCount,nearest = read_genes(plotGenes[0],resolution,chromosome,start,end)
			plength = (end-start)*float(resolution)/1000000
		...
		for peer1 in range(0,len(files)):
			...
			for peer2 in range(0,len(files)):	
				...
				if peer1 != peer2:
					...
					with np.errstate(divide='ignore'): img=pax1.imshow((matrix),cmap=plt.get_cmap("bwr"),origin=orientation,interpolation="nearest",extent=(int(start or 1) - 0.5,\
																			  int(start or 1) + length - 0.5,int(start or 1) - 0.5,int(start or 1) + length - 0.5),aspect='auto')
		
					pax1.set_ylim(int(start or 1) - 0.5,int(start or 1) + length - 0.5)
					pax1.set_xlim(int(start or 1) - 0.5,int(start or 1) + length - 0.5)
					pax1.set_title(('(%s / %s)') % (names[peer1],names[peer2]))
	...
	if inline:
		...
		if len(oExtension) > 0 and oExtension in list(plt.gcf().canvas.get_supported_filetypes().keys()): 
			extension='.'+oExtension
		elif 'JPEG' in list(plt.gcf().canvas.get_supported_filetypes_grouped().keys()) or 'Joint Photographic Experts Group' in list(plt.gcf().canvas.get_supported_filetypes_grouped().keys()): extension='.jpeg'
		else: extension = '.png'
